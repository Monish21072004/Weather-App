document.addEventListener('DOMContentLoaded', () => {
    ...
    function initMap() {
        if (!STATE.map) {
            STATE.map = L.map('explore-map').setView([20, 0], 2);
    ...
    function toggleMapView(showMap) {
        ...
        if (showMap) {
            ...
            initMap();
            // Must invalidate size after container becomes visible
            setTimeout(() => {
    ...
    async function scanGlobalWeather() {
        ...
        try {
            ...
            STATE.currentScanResults = results; // Store for map
            renderExploreGrid(results);
            if (STATE.isMapView) renderMapMarkers(); // Update map if visible
    ...
    async function scanLocalWeather(lat, lon) {
        ...
        // Generate 8 points around user (approx 15km radius)
        // 0.15 deg lat ~= 16km
        const offset = 0.15;
        const sectors = [
        ...
        try {
            ...
            STATE.currentScanResults = results; // Store for map
            renderExploreGrid(results);
            if (STATE.isMapView) renderMapMarkers();
        } catch(e) { console.error(e); }
    }
